...
class IMAGENET(torchvision.datasets.ImageNet):
    ...
    def __init__(self, root, imb_type='exp', imb_factor=0.01, rand_number=0, train=True,
                 transform=None, target_transform=None,
                 download=False):
        # root: str, split: str = 'train', download: Optional[str] = None,
        # train_data = torchvision.datasets.ImageNet("./dataset",
        #                                            split=train = True, transform = torchvision.transforms.ToTensor())
        # train_loader = DataLoader(dataset=train_data, batch_size=64, shuffle=True, num_workers=0, drop_last=True)
        super(IMAGENET, self).__init__(root, split='train', download=download,transform =transform)
        np.random.seed(rand_number)
        img_num_list = self.get_img_num_per_cls(self.cls_num, imb_type, imb_factor)
        self.gen_imbalanced_data(img_num_list)
...
if __name__ == '__main__':
    ...
    # import os
    # print(os.getcwd())
    # trainset = IMAGENET(root='/home/luanqz/hzy/CausalFlAggregation/imagenet/', train=True,
