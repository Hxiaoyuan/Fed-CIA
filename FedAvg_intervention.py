...
from util.fedavg import FedAvg_Rod, FedAvg_Rod_intervention
import sys
...
from intervention import InterventionModule, InterventionModel
from chest.validate import globaltest15, APH
...
def local_train(q, g, args, _train, idx, gpu_id):
    ...
    # print('Epoch {}/{} \t user:{} \t loss={}'.format(rnd, args.rounds, idx, loss_local))
    q.put([backbone_w_local.state_dict(), loss_local, _vec.cpu(), _acc, idx])
    return

def intervention_train(q, int_model, args, rnd):
    device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    module = InterventionModule(args=args, interModel=int_model, tail_dim=48, device=device)
    w = module.train(grads_tail, global_tail_vec=global_tail_vec)
    do_c=[]
    if rnd % args.do_frequency == 0:
        do_c = module.intervention_cifar10(grads_tail, global_tail_vec, inter_num=args.intervention_num_users)
    q.put([w.state_dict(), do_c])
    return
...
if __name__ == '__main__':
    ...
    alist = np.array(
        [[np.sum(labels[list(dict_users[i])] == j) for j in range(classes_num)] for i in range(args.num_users)])
    print("training set distribution:")
    print(alist)
    # print('dict_users:{}'.format(dict_users))
    ...
    test_loader = dataloader.DataLoader(xrayDataset(test_dateset, True, [i for i in range(len(test_dateset))], classes), batch_size=16, shuffle=False,
    ...
    interventionModel = InterventionModel(args=args, tail_dim=48, grad_dim=grad_dim, tail_class=tail_classed)
    checkpoint_path = args.checkpoint
    if checkpoint_path != '':
        checkpoint = torch.load(checkpoint_path)
        g_backbone.load_state_dict(checkpoint['state'])
        start_epoch = checkpoint['epoch']
        interventionModel.load_state_dict(checkpoint['inter_state']),
    else:
        start_epoch = 1
    ...
    for rnd in tqdm(range(start_epoch, args.rounds+1)):
        grads_tail=[0 for i in range(args.num_users)]
        ...
        print('\nEpoch {}/{} \t loss={}'.format(rnd, args.rounds, sum(batch_loss)))

        start_time = time.time()
        torch.cuda.empty_cache()
        q_int = mp.Manager().Queue()
        p_int = mp.Process(target=intervention_train, args=(q_int, copy.deepcopy(interventionModel), args, rnd, grads_tail, global_tail_vec))
        p_int.start()
        p_int.join()
        [w, do_c] = q_int.get()
        del q_int
        torch.cuda.empty_cache()
        interventionModel.load_state_dict(copy.deepcopy(w))

        if rnd % args.do_frequency == 0:
            dict_len = [len(dict_users[_idx]) for _idx in idxs_users]
            _grad_con = []
            for i, w_i in enumerate(idxs_users):
                if w_i in do_c:
                    dict_len[i] = 0
            backbone_w_com_params, backbone_w_tail_grad = FedAvg_Rod_intervention(backbone_w_locals, dict_len,
                                                                                  grads_tail, args.device)
        ...
        global_tail_vec = backbone_w_tail_grad
        # thread test
        torch.cuda.empty_cache()
        ...
        state = {
            ...
            'inter_state': interventionModel.state_dict(),
        }
        if not os.path.exists('./chest_save_model/model_15_pre_features_inter/1/train_dataset{}/{}'.format(args.chest_data,args.model)):
            os.makedirs('./chest_save_model/model_15_pre_features_inter/1/train_dataset{}/{}'.format(args.chest_data,args.model))
        checkpoint_path = os.path.join('./chest_save_model/model_15_pre_features_inter/1/train_dataset{}/{}/checkpoint_train_{}_user_{}_inter_{}_lr_{}_epoch_'
                                       .format(args.chest_data, args.model, args.chest_data, args.num_users, args.intervention_num_users, args.lr) + str(rnd) + '.pth.tar')
